...
- Phase 3N per-role receipt routing has created approved-but-unexecuted ActionItems for Engineering, Product, Security, and Operations; the Security owner blocker is resolved by founder appointment of 安全评审员甲, but the actual Security receipt content is still pending
- runtime adoption preflight remains No-Go when Computer Use cannot provide a same-session browser proof and role-level receipt evidence is still outside a separate route adoption review
- no auto-send
- no silent write
...
| 日期 | 变化 |
|---|---|
| 2026-05-29 | Runtime adoption preflight 与 Computer Use browser path packet 落地：新增 `docs/reviews/HELM_OPERATING_SIGNAL_FLOW_RUNTIME_ADOPTION_PREFLIGHT_AND_BROWSER_PATH_PACKET.md`，记录本轮 Computer Use 可发现 app 但 Chrome / Safari key-window 返回 `cgWindowNotFound`、Atlas timeout、重启 bundled service 后本 Codex turn transport closed，因此不能把 Computer Use 写成浏览证明；结论继续 No-Go for route adoption，`/operating` 默认层仍必须客户资产优先、fixture-backed、只读复核，runtime shadow 只能进入 review packet，不授权 schema/API、production query adoption、official write、自动执行、自动外发、移除 fixture banner 或 LLM final ranking |
| 2026-05-29 | Runtime shadow detail 纯投影落地：`lib/operating-signal-flow/projection.ts` 新增 `buildOperatingSignalFlowSnapshotDetailDisplayModel`，让已脱敏 `current_window` shadow snapshot 可以生成单条经营信号详情 display model，并把 shadow-only 行转成客户可读的行动候选、复核任务、审计回执和当前工作区经营闭环；该能力只用于 route-disconnected projection / targeted test，不接 `/operating` 或 `/operating/signals/[signalKey]` 默认路由，不授权 runtime UI adoption、schema/API、production query adoption、official write、自动执行、自动外发、移除 fixture banner 或 LLM final ranking |
| 2026-05-28 | Phase 2 fixture UI 深化：`/operating` 最高压路径改为进入单条经营信号全生命周期详情页 `/operating/signals/[signalKey]`，详情页只读展示客户材料、客户资产、当前停住点、证据、AI 工作姿态和 source -> object -> judgement -> human boundary -> receipt -> memory 全链路；真正承接动作仍回落到 allowed next action 白名单（如 `/approvals`），移动端增加“信号链路”入口；继续禁止 schema/API/runtime query/production query adoption/official write/自动执行/自动外发/LLM final ranking |
| 2026-05-21 | Phase 3N Security receipt owner appointment readout packet 落地：新增 `docs/reviews/HELM_OPERATING_SIGNAL_FLOW_PHASE3N_SECURITY_RECEIPT_OWNER_APPOINTMENT_READOUT_PACKET.md`，按 founder 决策将 Security receipt owner / approver 指定为安全评审员甲，appointment audit 为 `cmpfhhni800011yi9xx4bb2o6`；post-appointment shadow probe 为 single-workspace、boundary 0、pendingReviewCount 0、eventCount 116、digest `8d648366be0ec4f83aec33fa7952398099d60e03205eb9e69d404637c172a175`，继续禁止 route adoption、fixture banner removal、production default flag、schema/API、official write、自动执行、自动外发或 LLM final ranking |

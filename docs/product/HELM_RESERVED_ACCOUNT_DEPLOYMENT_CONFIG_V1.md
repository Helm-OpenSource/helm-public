...
- `helm@zhaojiling.com` 是 Helm 预留账户的主账号锚点，也是默认系统发信账号。
- 运行时真正决定 Helm 预留租户的是 `Workspace.workspaceClass = HELM_RESERVED` 与 `Workspace.systemKey = "helm_reserved_primary"`。
- `ALIYUN_MAIL_SYSTEM_EMAIL=helm@zhaojiling.com` 只决定系统邮件 sender，不决定当前请求属于哪个 workspace。
...
- `helm@zhaojiling.com` 作为生产主账号应具备该 reserved workspace 的 `OWNER` 或等效运营成员身份。
...
| 系统邮箱 | `lib/notifications/system-mail.ts` 与 `.env.example` | 默认 sender 为 `helm@zhaojiling.com`，不参与 workspace routing |
...
ALIYUN_MAIL_SYSTEM_EMAIL="helm@zhaojiling.com"
...
WHERE u.email = 'helm@zhaojiling.com'
...
3. 创建或确认 `helm@zhaojiling.com` 用户，并将其加入 Helm reserved workspace。
4. 配置 Aliyun system mail secrets，确保 `ALIYUN_MAIL_SYSTEM_EMAIL=helm@zhaojiling.com`。

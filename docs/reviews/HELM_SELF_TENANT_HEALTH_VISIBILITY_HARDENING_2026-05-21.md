...
本轮移除了 customer workspace carveout：示例客户等普通客户租户不再显示“租户健康”入口，也不能直接访问 `/operating/tenant-health`。该能力只允许 `Workspace.workspaceClass = HELM_RESERVED` 且 `systemKey = helm_reserved_primary` 的 Helm reserved workspace 使用。
...
- `assertTenantHealthAccess(...)` 因此对示例客户等 `CUSTOMER` workspace fail closed。
...
- customer workspace 即使 slug/systemKey 为示例客户，也不可访问 tenant health。
...
生产仍需部署新包后，示例客户租户界面才会消失该入口。代码与远端 main 收口后，按 zip/SFTP 发布流程部署。

...
### OPTIONAL_SCHEDULERS（可选，默认全部关闭）

| 变量 | 用途 |
|---|---|
| `SIGNAL_COLLECTION_SCHEDULER_ENABLED` | 信号采集调度器；`true` 才启动 |
| `LIGHT_CHAIN_FOLLOW_THROUGH_CRON_ENABLED` | 轻量任务链催办扫描（每日一次）：对逾期 / 停留的任务与承诺产出**内部提醒与审计**，不外发、不改任何任务状态。部署级开关，会扫描库内所有工作区；建议先在内部自用工作区验证节奏，参考[自身租户运营 runbook §5](./product/HELM_SELF_TENANT_OPERATING_RUNBOOK.md) |
...
| 变量 | 用途 |
|---|---|
| `HELM_PUBLIC_OPERATOR_DISPLAY_NAME` | `/terms` 与 `/privacy` 展示的部署运营方简称；不等同于法定名称 |
| `HELM_PUBLIC_OPERATOR_LEGAL_NAME` | 法定运营主体名称；只在登记证据已由部署方私有发布流程核验后填写 |
| `HELM_PUBLIC_OPERATOR_REGISTRATION_VERIFIED` | 仅接受 `true` / `false`；只有精确为 `true` 时页面才显示法定名称 |

默认保持租户中立且 `false`。如果 verification 为 `true` 但法定名称缺失，或填写了
其他模糊真值，`npm run validate:env` 会失败。公开 Core 不携带任何公司的工商登记
证明；正式上线还需要部署方完成法律文本、隐私政策、备案、域名和联系渠道复核。

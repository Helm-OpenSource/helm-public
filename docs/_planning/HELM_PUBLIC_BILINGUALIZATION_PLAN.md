...
3. 抽查 264 个 `.tsx` 文件后，少量硬编码英文 UI 标签已修正；workspace runtime
   ...
   conversation capture 服务层的 fallback transcript / speaker label / audit summary 文案已继续收口。
...
3. After sampling 264 `.tsx` files, a small set of hard-coded English UI labels
   ...
   service fallback transcript / speaker-label / audit-summary copy, have also been localized

...
第三个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/mainline-ledger.ts` 定义 append-only 私有主线账本事件、
  当前有效视图、review finding 资产化 / owner 豁免、冲突键 supersede 约束和只读 append plan。
- `features/work-unit-governance/work-unit-mainline-ledger-panel.tsx` 和
  `/demo/work-unit-governance` 展示公司主线账本的追加记录与当前有效视图，仍只使用 synthetic fixture。
- `scripts/check-work-unit-governance.ts` 已增加 ledger fixture，覆盖 Public Core 不写私有主线、
  冲突键重叠必须带 supersede / baseline 证据、AI 不能代 owner 豁免 review finding。

第四个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/owner-lifecycle.ts` 定义负责人复核回执、授权代理回执、超时升级、
  负责人变更计划和只读 owner lifecycle readout；所有计划均标记为 Public Core 不持久化、
  不通知、不外发、不授权、不生效。
- `features/work-unit-governance/work-unit-owner-lifecycle-panel.tsx` 和
  `/demo/work-unit-governance` 展示低复杂度“需要确认的人”路径，业务用户只看到待确认人、
  已确认人、待处理和建议下一步，不暴露内部工程术语。
- `scripts/check-work-unit-governance.ts` 已增加 owner lifecycle fixture，覆盖 AI 不能记录 owner
  决定、授权代理必须有当前人类 owner 授权回执、主线漂移后需要重新复核、公开 Core 不自动通知
  或批准。

第五个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/activation-handoff.ts` 定义生效交接请求、独立授权回执形状、只读
  activation handoff readout 和动作计划；所有计划均标记为 Public Core 不持久化、不外发、
  不写目标系统、不授予批准、不触发 runtime 生效。
- `features/work-unit-governance/work-unit-activation-handoff-panel.tsx` 和
  `/demo/work-unit-governance` 展示低复杂度“生效范围交接”路径，用户只看到生效范围、恢复方式、
  回执形状和建议下一步，不暴露内部工程术语。
- `scripts/check-work-unit-governance.ts` 已增加 activation handoff fixture，覆盖未进入公司主线
  不得申请生效、AI 不得记录生效授权、客户可见 / 商业承诺默认需要更正或补救计划、主线漂移后
  需要重新复核、公开 Core 不外发 / 不写回 / 不生效。

第六个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/repair-learning-loop.ts` 定义 AI 修复候选记录、学习发现、
  学习资产草案、只读 repair learning readout 和动作计划；所有计划均标记为 Public Core
  不持久化、不外发、不写回、不授予批准、不触发 runtime 生效、不自动修改检查规则。
- `features/work-unit-governance/work-unit-repair-learning-panel.tsx` 和
  `/demo/work-unit-governance` 展示低复杂度“修复与经验入库”路径，用户只看到失败检查、
  新候选方案、经验资产和建议下一步，不暴露内部工程术语。
- `scripts/check-work-unit-governance.ts` 已增加 repair learning fixture，覆盖 AI 修复必须回到
  新候选方案、AI 不得通过修改检查规则让自己通过、每个 finding 必须落为可执行资产或人类 owner
  豁免、公开 Core 不外发 / 不写回 / 不生效 / 不应用经验资产。

第七个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/proof-package.ts` 定义快照绑定的证明包、证据条目、HWU 覆盖矩阵、
  只读 proof package readout 和 public-safe 红action约束；所有证明包均标记为 Public Core
  不持久化、不外发、不写回、不授予批准、不授予 readiness、不触发 runtime 生效、不应用经验资产。
- `features/work-unit-governance/work-unit-proof-package-panel.tsx` 和
  `/demo/work-unit-governance` 展示低复杂度“证明包查看器”，用户只看到证据条目、已覆盖需求、
  快照绑定和边界说明，不暴露内部工程术语。
- `scripts/check-work-unit-governance.ts` 已增加 proof package fixture，覆盖快照错配必须阻断、
  raw private / unknown 红action状态不得进入公开证明包、HWU 覆盖矩阵不得缺项、证明包不能被当成
  readiness / approval / activation。

第八个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/private-mainline-store.ts` 定义私有主线 store 绑定契约、
  append handoff envelope 和 Public Core no-op 默认 store。私有绑定必须声明 append-only、
  snapshot-bound receipt、冲突键串行化、人类 owner 回执和独立 activation authority；Public Core
  默认 store 只能返回阻断，不写私有主线事实。
- `lib/work-unit-governance/private-mainline-store.test.ts` 覆盖私有 store envelope 只是交接包、
  不是 readiness；Public Core no-op store 不得 append；缺少治理能力的私有绑定必须 fail closed；
  任何 public-core 绑定声称保存真实实例、持久化、写主线、外发或 runtime 生效必须阻断。
- `scripts/check-work-unit-governance.ts` 已增加 private mainline store fixture，覆盖 store 绑定
  不得把 Public Core 变成真实持久化 / 外部副作用执行面。

第九个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/owner-notification-binding.ts` 定义 owner 通知 / 升级的私有调度器
  绑定契约、handoff envelope 和 Public Core no-op 默认 dispatcher。私有绑定必须声明只传递
  owner refs、快照绑定消息、人类 owner 授权、升级策略、联系方式留在私有面、红action envelope、
  私有发送能力和私有回执持久化能力；Public Core 默认 dispatcher 只能返回阻断，不发送通知。
- `lib/work-unit-governance/owner-notification-binding.test.ts` 覆盖通知 envelope 只是交接包、
  不含联系方式、不是 readiness / approval；Public Core no-op dispatcher 不得 dispatch；缺少治理能力
  的私有绑定必须 fail closed；任何 public-core 绑定声称通知、持久化、写目标、批准或 runtime 生效
  必须阻断。
- `scripts/check-work-unit-governance.ts` 已增加 owner notification fixture，覆盖通知 / 升级绑定不得把
  Public Core 变成真实发送、外部副作用、批准或运行时执行面。

第十个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/activation-runtime-binding.ts` 定义生效运行时执行器的私有绑定契约、
  execution envelope 和 Public Core no-op 默认执行器。私有绑定必须证明快照绑定执行、
  主线回执、人类 owner 授权、独立生效授权、回退 / 更正计划、目标引用在 Public Core 外解析、
  私有执行器负责 runtime 生效，以及私有面持久化执行回执。
- `lib/work-unit-governance/activation-runtime-binding.test.ts` 覆盖 execution envelope 只是交接包、
  不是 readiness / activation；Public Core no-op 执行器不得 execute；缺少治理能力的私有绑定
  必须 fail closed；任何 public-core 绑定声称持久化、写目标、批准或 runtime 生效必须阻断；
  原始网络目标引用必须阻断且不得回显真实目标。
- `scripts/check-work-unit-governance.ts` 已增加 activation runtime fixture，覆盖生效执行绑定不得把
  Public Core 变成真实执行、目标写入、批准或运行时生效面。

第十一个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/learning-asset-store-binding.ts` 定义经验资产私有 store 绑定契约、
  learning-asset envelope 和 Public Core no-op 默认 store。私有绑定必须证明 finding 存在、
  草案已验证、资产快照绑定、owner 豁免需要人类回执、应用策略存在、私有面负责持久化，
  以及私有面负责应用经验资产。
- `lib/work-unit-governance/learning-asset-store-binding.test.ts` 覆盖 learning-asset envelope 只是交接包、
  不是 readiness / applied asset；Public Core no-op store 不得 record / apply；缺少治理能力的私有绑定
  必须 fail closed；任何 public-core 绑定声称持久化、外发、写目标、修改检查规则、应用经验资产
  或批准必须阻断；原始网络资产引用必须阻断且不得回显真实目标；AI 不得请求权威落库或应用。
- `scripts/check-work-unit-governance.ts` 已增加 learning asset store fixture，覆盖经验资产落库 / 应用绑定
  不得把 Public Core 变成真实持久化、检查规则修改、批准或资产应用面。

第十二个 Public Core 工程切片已锚定：

- `lib/work-unit-governance/private-plane-handoff.ts` 聚合 private mainline store、owner notification、
  activation runtime executor 和 learning asset store 四类 Public Core no-op 绑定，生成只读私有面
  交接 readout；所有 lanes 均标记为只准备交接、不持久化、不外发、不写目标、不授予批准、不生效、
  不应用经验资产。
- `features/work-unit-governance/work-unit-private-plane-handoff-panel.tsx` 和
  `/demo/work-unit-governance` 展示低复杂度“私有执行面交接边界”，业务用户只看到四个交接对象、
  私有面职责、公开 Core 边界和建议下一步，不暴露 GitHub 内部术语。
- `features/work-unit-governance/work-unit-private-plane-handoff-panel.test.tsx` 和 `test:public:guards`
  覆盖四类交接对象必须同时可见，且页面不得把交接误表达成生产就绪、执行、生效、批准、
  经验资产已应用或真实私有信息泄漏。

截至 2026-07-19，本需求定义的治理契约与私有面交接证明已闭环：Public Core 侧覆盖
HWU-01 到 HWU-15 的 schema、guard、测试、synthetic fixture、只读运行时读出、只读 append plan、
owner lifecycle plan、activation handoff plan、repair learning plan、proof package readout、private mainline
store 绑定 / 交接契约、owner notification 绑定 / 交接契约、activation runtime 绑定 / 交接契约、
learning asset store 绑定 / 交接契约、private-plane handoff readout 和 demo 复核面；私有 companion
仓侧已落地 metadata-only private receipt index、private receipt ledger、owner notification dispatcher、
activation runtime executor 和 learning asset store 五类 seam，证明四条私有 lane 可在不把客户事实、
目标坐标、联系方式、凭据或原始资产内容放入 Public Core 的前提下写入私有回执。

这仍不代表客户部署、生产连接器、release readiness、商业承诺、自动审批或自动对外发送已经成立。
真实 tenant runtime 仍必须在所属私有控制面单独验证 owner authority、activation authority、composition
pin、执行适配器、回退 / 更正回执、观测性和恢复演练；Public Core 不保存客户真实主线事实，也不成为
runtime 执行平面。
...
- 检查通过不能自动定稿。
- 定稿不能自动生效。
- AI 修复只能形成新候选方案，不能自动接受、自动定稿或修改检查规则让自己通过。
...
- 证明包必须快照绑定、public-safe，并明确不是 readiness、approval 或 activation。
- 私有主线 store 绑定必须 fail closed；Public Core 默认 store 不得保存真实主线事实或执行副作用。
- 生效运行时执行器绑定必须 fail closed；Public Core 默认执行器不得写目标、批准、持久化执行回执或触发 runtime 生效。
- 经验资产 store 绑定必须 fail closed；Public Core 默认 store 不得持久化、外发、修改检查规则、应用经验资产或批准。

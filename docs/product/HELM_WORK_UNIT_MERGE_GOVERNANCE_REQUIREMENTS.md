...
这些切片仍不代表 HWU-01 到 HWU-15 全量实现完成。真实私有主线账本持久化、tenant / overlay 侧真实
owner 通知分发和升级回执、tenant / overlay 侧真实 activation runtime 执行与私有 execution receipt
持久化、tenant / overlay 侧真实 learning asset store 执行、私有学习资产回执持久化和自动应用仍属于后续切片。
Public Core 仍只保存 schema、guard、测试、synthetic fixture、只读运行时读出、只读 append plan、
...
learning asset store 绑定 / 交接契约、private-plane handoff readout 和 demo 复核面。

import askHelmQueryIntentCases from "@/evals/ask-helm/query-intents.json";
import {
  ASK_HELM_INTENT_TYPES,
...
export type AskHelmQueryIntentEvalCaseResult = {
  id: string;
  query: string;
  expectedIntentType: AskHelmIntentType;
...
export type AskHelmQueryIntentEvalSummary = {
  ...
  cases: AskHelmQueryIntentEvalCaseResult[];
};

export function loadAskHelmQueryIntentCases() {
  return (askHelmQueryIntentCases as AskHelmQueryIntentSample[]).map((item) => {
...
export function runAskHelmQueryIntentEval(
  minimumPassRate = 80,
): AskHelmQueryIntentEvalSummary {
  const results = loadAskHelmQueryIntentCases().map<AskHelmQueryIntentEvalCaseResult>(
    (item) => {
  ...
  return {
    ...
    byIntent: ASK_HELM_INTENT_TYPES.map((intentType) => {

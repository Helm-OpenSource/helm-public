...
- GitHub 社区与维护入口补齐：`SUPPORT.md`、`CODEOWNERS`、issue forms、PR template、`dependabot.yml`、CodeQL workflow、Docker quickstart smoke workflow
- Business Asset detail route：新增 `/assets/[assetType]/[assetId]` 与 `features/business-assets/*`，把客户、机会、承诺和风险统一投影成可复核的经营资产详情；继续保持 workspace-scoped、review-first、no official write。
- Operating Signal detail route：新增 `/operating/signals/[signalKey]` 和 route-disconnected runtime shadow detail projection，单条经营信号可以展示脱敏生命周期、handoff 和证据链；runtime adoption 仍为 No-Go，不移除 fixture boundary。
...
- 主路径和二级经营页面首屏同步 source public mirror 的客户资产聚焦升级：`/opportunities`、`/approvals`、`/inbox`、`/reports`、`/imports`、`/settings`、`/diagnostics`、`/analytics` 默认前置经营资产、当前压力、待判断和下一步路径，说明性摘要进入 `LazyDisclosure` 折叠层；不改变 schema/API、权限、自动执行、自动审批或外发能力。
- `/health` 增强公开健康页的健壮性：固定 `nodejs` runtime，metadata 不读取 request cookie，readout 生成失败时降级为 public-safe fallback，避免健康页因请求态 helper 或审计摘要异常崩溃。
- 公开用户入口文档对齐当前仓库：README / CONTRIBUTING / getting-started / public-trial runbook / launch draft 统一改为 `Helm-OpenSource/helm-public`，并把 quickstart 固定为默认公开上手路径
- GitHub Actions 主工作流显式收紧为最小 `contents: read` 权限，并为公开仓库新增 supply-chain / static-analysis 自动化入口
...
- 公开仓库不再包含 `extensions/guangpu/`、`app/api/extensions/guangpu/` 与租户专属脚本（按 [docs/internal/HELM_PRIVATE_TENANT_SEPARATION_PLAN_V1.md](docs/internal/HELM_PRIVATE_TENANT_SEPARATION_PLAN_V1.md) 双仓库路径执行）

...
| **RUNTIME_ATTESTATION** | `HELM_HEALTH_ATTESTATION_MODE`, `HELM_RUNTIME_DEPLOYMENT_ID` | Defaults to a public reachability check; source-built releases can explicitly select `artifact-bound`, inject a non-secret opaque ID through the private release process, and make `/api/health` return `503` when the ID is missing or invalid |

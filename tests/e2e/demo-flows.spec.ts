...
const loadingHeadingPattern =
  /^(正在打开工作区页面|Opening the workspace surface|正在准备当前页面|Preparing this page)$/u;
...
test("公开首页采用左侧品牌、三条起始线与顶部极简切换", async ({ page }) => {
  ...
  await expect(page.getByTestId("home-path-fork")).toBeVisible();
...
async function expectDesktopFirstVisibleHeadingIsPageTitle(page: Page) {
  await waitForWorkspaceUiHydration(page);
  await expect(visibleHeadings(page).first()).not.toHaveText(loadingHeadingPattern);
  ...
  expect(pageTitleText).not.toMatch(loadingHeadingPattern);
...
test("founder 主演示链路可以从首页进入会议再到审批", async ({ page }, testInfo) => {
  ...
  const approvalsFocus = page.locator('[data-customer-asset-focus="true"]').first();
  await expect(approvalsFocus).toBeVisible();
  await expect(approvalsFocus).toContainText(/客户可见草稿|Customer-visible draft/);
  await expect(approvalsFocus).toContainText(/需要你判断|Decision/);

  const meetingDrafts = page.locator("#meeting-follow-through-review");
  await expect(meetingDrafts).toBeVisible();
  await expect(meetingDrafts).not.toHaveAttribute("open", "");
  await expect(meetingDrafts).toContainText(/其他会议草稿|Other meeting drafts/);
  await meetingDrafts.getByText(/展开草稿|Open drafts/).click();
  await expect(meetingDrafts).toHaveAttribute("open", "");
  await expect(
    meetingDrafts.getByText(/会议会后动作确认|Meeting follow-through review/),
  ).toBeVisible();
  await expect(
    meetingDrafts.getByRole("button", { name: /打开第一条草稿|Open first draft/ }),
  ).toBeVisible();
...
test("机会高级筛选可用", async ({ page }) => {
  ...
  await page.getByTestId("opportunity-filter-disclosure").locator("summary").click();
...
test("主路径首屏先给客户资产焦点", async ({ page }) => {
  await loginAs(page, "founder@demo.com");

  await page.goto("/approvals");
  await waitForWorkspaceUiHydration(page);
  const approvalsFocus = page
    .locator('[data-customer-asset-focus="true"]')
    .first();
  await expect(approvalsFocus).toBeVisible();
  await expect(approvalsFocus).toContainText("客户可见草稿");
  await expect(approvalsFocus).toContainText("需要你判断");

  await page.goto("/inbox");
  await waitForWorkspaceUiHydration(page);
  const inboxFocus = page.locator('[data-customer-asset-focus="true"]').first();
  await expect(inboxFocus).toBeVisible();
  await expect(inboxFocus).toContainText("客户线程");
  await expect(inboxFocus).toContainText("下一步判断");

  await loginAs(page, "saleslead@demo.com");
  await page.goto("/opportunities");
  await waitForWorkspaceUiHydration(page);
  const opportunitiesFocus = page
    .locator('[data-customer-asset-focus="true"]')
    .first();
  await expect(opportunitiesFocus).toBeVisible();
  await expect(opportunitiesFocus).toContainText("客户资产");
  await expect(opportunitiesFocus).toContainText("当前状态");
  await expect(opportunitiesFocus).toContainText("下一步判断");
});
...
test("可以载入本地阿里邮箱示例数据并在收件箱看到真实来源标识", async ({ page }) => {
  ...
  if (await mockButton.isVisible().catch(() => false)) {
    ...
    await expect(
      page.getByText(/已接入本地模拟阿里邮箱数据|Local mock Aliyun data connected/).first(),
    ...
    await expect(
      page.getByText(/阿里邮箱已完成同步|已同步本地 mock 阿里邮箱数据|Aliyun mail synced|Local mock Aliyun Mail data synced/).first(),
    ).toBeVisible({ timeout: 30_000 });
  }
  ...
  await expect(page.getByText(/当前只读：阿里邮箱不会回写或发送邮件。/)).toBeVisible();
});

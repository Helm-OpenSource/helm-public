...
**License**：Apache-2.0 · **仓库姿态**：open-source Core 定位，visibility flip 只在 public-release gate 与 owner Go/No-Go 之后执行 · **Helm Cloud / Enterprise**：商业版可选，不替代开源
...
1. 看 `extensions/case-management-sample/fixtures/case.sample.json`。
2. 改一条 synthetic fixture。
3. 跑 `npm run pack:fixture-check` 与 `npm run eval:headless-signal-interface`。
4. 回到 `/demo` / `/operating` 看判断、证据、复核、边界是否仍保持 review-first。

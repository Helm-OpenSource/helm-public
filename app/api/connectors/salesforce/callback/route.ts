...
export async function GET(request: Request) {
  ...
  if (error) {
    return NextResponse.redirect(new URL(`/imports/crm?provider=salesforce&status=oauth-error&message=${encodeURIComponent(error)}`, request.url));
  }

  if (!code || !state || !rawState) {
    return NextResponse.redirect(new URL("/imports/crm?provider=salesforce&status=missing-state", request.url));
  }
  ...
  if (!callbackContext.ok) {
    ...
    return NextResponse.redirect(
      new URL(
        `/imports/crm?provider=salesforce&status=${
  ...
  try {
    const token = await exchangeSalesforceCode(code);
    const source = await upsertSalesforceSourceFromOauth({
      workspaceId,
      userId,
      token,
    });
    ...
    return NextResponse.redirect(new URL(`/imports/crm?provider=salesforce&status=connected&sourceId=${source.id}`, request.url));
  } catch (callbackError) {
    const message = callbackError instanceof Error ? callbackError.message : "Salesforce connection failed";
    return NextResponse.redirect(
      new URL(
        `/imports/crm?provider=salesforce&status=error&message=${encodeURIComponent(message)}`,
        request.url,
      ),
    );
  }

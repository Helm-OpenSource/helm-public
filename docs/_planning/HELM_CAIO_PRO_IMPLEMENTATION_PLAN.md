...
### 4.4 部署完成门（V1 完成即待办清零）

P4-P8 不只是实施批次，还物化为一份受治理的初始化后待办清单
（`CAIO_PRO_V1_COMPLETION_ITEMS`，封闭有序的 13 项）。每一项要么由系统在同一
事务内从库中活算（资产状态齐备、G0 受理、10 题组合、CEO 选择、实施计划、
监督链闭合、单题价值回执、复盘回执），要么由 owner 经 CEO principal binding
记录不可变 attestation（盘点确认、设备安全、运行真值绑定、事故态势、
成熟度决定）。

规则：

- 只有当每一项都凭真实回执 satisfied、完成评估为
  `ready_for_owner_acceptance`、且 CEO 受理完成门回执后，才构成
  **现场部署完成**；任何一项缺失即 `not_ready`，没有部分通过。
- 完成门本身是治理记录：每张回执冻结
  `fullFunctionOperation = "not_authorized_by_this_receipt"`。受理只是
  **全功能工作**的前置条件，绝不是激活。
- **全功能工作**在完成门受理之后才可开始，且是独立的 owner 动作，
  由本计划之外的授权与激活门控承载。
- 单题价值回执的结果窗必须 ≥30 天；不足 30 天必须记录 shortfall 原因，
  且该回执永远不能借此满足清单项。模型输出数量、token 用量、报告篇数、
  接入系统数永远不是经营价值。
- 完成门可撤销；撤销后「现场部署完成」声明即刻失效，重新受理需要新的
  完成评估。

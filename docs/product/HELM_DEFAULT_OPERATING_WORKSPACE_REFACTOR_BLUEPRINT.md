...
| ③ 我的工位 | 按角色家目录渲染当前角色主区（≤4 入口）。**角色家分流是 `/dashboard` 内容层的视图选择，不是路由重定向**（§2.1 优先级链）：`controlTowerHome` 开启时，管理角色在 `/dashboard` 看到控制塔四段，非管理角色在同一 URL 看到其工位家视图（含直达工位的主区入口卡）；不发 redirect、不新增落地配置 | 附录 B 角色映射 |
...
新增的 `controlTowerHome` **仅是 `/dashboard` 的内容开关**（渲染控制塔视图或旧版视图），**不新增第二套落地配置**。

**完整优先级链（含角色分流，消除歧义）**——四层各管一件事、逐层生效：

1. **登录落点**：只由 `defaultLandingPath` 决定（现有机制，不变）。角色家路由**不参与**落点决策。
2. **`?stay=1` 逃生口**：只作用于 `/dashboard` 的 redirect 判定（压过 `defaultLandingPath`），与角色分流无关、**不压制角色分流**。
3. **`controlTowerHome` 内容开关**：决定 `/dashboard` 渲染新视图（控制塔/工位家）还是旧版视图。
4. **角色家分流**：仅在第 3 层选择了新视图后，在**内容层**决定渲染控制塔四段（管理角色）还是工位家视图（非管理角色）。同一 URL、无 redirect、无新配置；异常落 GENERIC 面（§1.4）。

完整真值表见附录 E；关键不变式：

- 任何组合下 `/dashboard?stay=1` 都能到达 `/dashboard` 本体（逃生口永不失效）；
- 不存在"配置合法但控制塔永远不可达"的组合（管理角色 + `controlTowerHome` 开 + 到达 `/dashboard` ⇒ 必见控制塔）；
- `defaultLandingPath` 非法值按现有 fail-closed 规则视同未配置；
- 角色分流永不产生额外 redirect，回滚（关 `controlTowerHome`）即整体回旧版视图。
...
1. **只读或只导航**：所有 surface 条目只有数据字段与 `href`（导航到既有人审流程）。**contribution descriptor 类型中不存在动作回调字段**。门禁在契约层用**类型系统或 AST 检查**拦截 descriptor 内的函数型动作字段——不做源码文本"禁 `on*`"（那会误伤折叠、键盘交互等正常 UI 代码）。**诚实边界**：这是 surface **形状约束**，不是执行隔离保证——`buildMainline` / `getAccess` 仍是进程内任意 provider 函数，当前 plugin runtime 没有真正 sandbox（AGENTS.md §6.1）。配套要求：`href` 必须通过站内路径 fail-closed 校验（沿 `resolveWorkspaceDefaultLandingPath` 的规则：仅站内绝对路径、拒协议/双斜杠），且目标页自行重新鉴权（导航不授权，§1.4）。
...
对基线 `79ef569` 的全部 52 个 `app/(workspace)` 页面文件做完整 inventory（逐页表见附录 A）：**50 页已定 A/B/C（A=19 文件、B=26、C=5），1 页待定（`mobile`，B/C 待人工审计），1 页为路由基础设施（workspace 根 redirect）**。

- **A 类 · 升舱为新范式一等公民**（19 个文件，约 10 个能力面）：`dashboard`（控制塔本体）、`operating`（信号层供体；`gtm-leads` / `tenant-health` 为 Helm reserved 工位）、`approvals`（复核工位，**永不进入退役批次**——review-first 红线的载体）、`opportunities`（推进工作台）、`customer-success`（交付工作台）、`inbox` / `meetings`（推进工作台动作源，撤独立导航）、`memory`（改进候选证据基座）、`imports`（接入面 = 冷启动一等公民）、`settings`。
- **B 类 · 事实面保留**（26 个文件，另含待定的 `mobile`）：撤出一切导航、仅深链可达，作为判断的证据面。含 `companies` / `contacts` / `conversations/[id]` / 八个共用同一机会商业详情数据源的 `[id]` 页（长期向 `assets` 规范详情或工作台抽屉归一）/ `reports`（**扩展宿主不可破**：`/reports?tab=` 深链契约原样保留）/ `search` / `analytics` / `diagnostics` / `capture` / `mobile`（待人工审计定 B 或 C）。
- **C 类 · 退役**（5 个文件；`mobile` 若审计为演示壳则升为 6）：`founder-qa/[id]`、`sales-followups/[id]`、`sales-objections/[id]`、`delivery-reviews/[id]`、`delivery-walkthroughs/[id]` 五个已核验的纯 redirect 兼容壳（各约 11 行，仅 `redirect()` 到 `/conversations/[id]` 或 `/follow-ups/[id]`）+ `mobile`（若审计为演示壳）。
...
**角色分流与本表的关系（第四维，内容层）**：上表所有"渲染控制塔"的格，对非管理角色实际渲染其工位家视图（同一 URL、内容层分流、无 redirect，见 §2 段③与 §2.1 优先级链第 4 层）；角色解析异常落 GENERIC 面。本表的落点结果不受角色影响——角色只改变 `/dashboard` 新视图内部渲染哪套内容。

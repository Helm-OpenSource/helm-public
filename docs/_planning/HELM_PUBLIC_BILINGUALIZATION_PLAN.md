...
3. 抽查 264 个 `.tsx` 文件后，少量硬编码英文 UI 标签已修正；workspace runtime
   ...
   evolution skill / strategy suggestion API 的失败 fallback 文案已继续收口。
...
3. After sampling 264 `.tsx` files, a small set of hard-coded English UI labels
   ...
   fallback copy, plus evolution skill / strategy suggestion API failure fallback
   copy, have also been localized

...
| Public Core P0 evidence -> memory candidate -> review packet chain | Formed as an offline public Core slice; still not production connector, CRM writeback, official memory, or customer deployment proof | [Headless Signal Interface requirements](product/HELM_HEADLESS_SIGNAL_INTERFACE_REQUIREMENTS.md), `extensions/case-management-sample/signals/review-packet.ts`, `npm run eval:headless-signal-interface` |

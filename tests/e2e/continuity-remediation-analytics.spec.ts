...
test.describe.serial("continuity remediation analytics surfaces", () => {
  ...
  test("operating surface shows review-required repeat-pattern analytics without widening authority", async ({ page }) => {
    ...
    await expect(page.getByText("PR25 review repeat remediation").first()).toBeVisible();
    await expect(page.getByText(/pilot risk /i).first()).toBeVisible();
    await expect(page.getByText(/pilot LOW\/1/).first()).toBeVisible();
    await expect(page.getByText(/sop Protected-field review SOP/).first()).toBeVisible();
    await expect(page.getByText(/runbook Stop repeat remediation loop/).first()).toBeVisible();
    await expect(page.getByText(/effect NO_SIGNAL/).first()).toBeVisible();
    await expect(page.getByText(/protected-field review/i).first()).toBeVisible();
  });
});

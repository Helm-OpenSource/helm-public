...
**License**：Apache-2.0 · **首次公开发布**：`v0.1.0-trial`（2026-05-31 目标） · **Helm Cloud / Enterprise**：商业版可选，不替代开源
...
| 30 分钟跑通完整闭环 | 否 | 否 | **`npm run quickstart`** |
...
如果你想先确认 Docker / compose / 端口前置是否齐全，再决定是否启动：
...
这条路径默认适合“本机没有 MySQL，只想先把 Helm 全系统跑起来”的开发者。它会检查 Docker daemon、compose 和 `3000` / `3306` 端口，然后直接启动仓库内置的 `mysql:8.4 + app`。
...
| [SUPPORT.md](SUPPORT.md) | 先看排障与提问入口；如果 Discussions 尚未启用，默认走 Issues |
| GitHub Discussions / `integration:` issue | 商业合作、联合发布、集成共建先走公开讨论；正式合作入口在 release closeout 后单独公布 |
...
| **🆘 [SUPPORT.md](SUPPORT.md)** | 先看排障、自助检查和提问方式；安全问题不要走公开 issue |

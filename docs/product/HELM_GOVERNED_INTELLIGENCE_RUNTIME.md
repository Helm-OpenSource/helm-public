...
- 从已验证 LLM 候选到现有 Artifact review 的 generic materializer 已实现；人工晋级
  governed action 的显式桥仍未实现。
...
PR6 增加的 `GovernedJudgementCandidate` 是 materialization 入站契约，不是动作授权：

- strict schema 绑定 source candidate / bundle / build receipt、verified evidence、
  judgement proposal、三角色结果、完整 trajectory 与 canonical content hash；
- trajectory 必须 read-only，且不得声明 release-ready、approval、promotion 或
  source-truth self-certification；
- materializer 需要 workspace-scoped `draft_only` `CapabilityGrant` 与 kill-switch ref；
- 单事务只允许创建 `ArtifactBundle(DRAFT)`、`ArtifactReview(PENDING)` 和最小审计行；
- 确定性 candidate ID 提供幂等；跨 workspace collision、未知字段、hash / evidence
  mismatch、PII / URL / credential-shaped text 全部 fail closed；
- 不创建 `ApprovalRequest`、`ActionItem`、`ApprovalTask`、feedback、preference、
  pattern、memory 或 official-write intent。

PR6 的 kill-switch 校验只证明不可缺少的 ref 已绑定；engaged-state 查询与撤销元数据由
PR8 control-plane contract 接入，当前无 live intake caller。Canonical hash 使用排序 key、
UTF-8 JSON 字符串和 safe-integer-only 数值的 JavaScript 定义；其他语言 verifier 尚未声明成立。

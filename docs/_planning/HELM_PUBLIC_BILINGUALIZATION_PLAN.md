...
3. 抽查 264 个 `.tsx` 文件后，少量硬编码英文 UI 标签已修正；workspace runtime
   ...
   复核卡片 / 步骤列表、recommendation feedback、UI primitive aria fallback、guidance preferences、home surface arrival banner
...
3. After sampling 264 `.tsx` files, a small set of hard-coded English UI labels
   ...
   recommendation feedback, UI primitive aria fallback, guidance preferences, home surface arrival banner, and billing program
...
| 2026-06-04 | 继续收口 meeting v2 接入与检索卡片的中文空状态混写；本批只改连接器接入与检索轨迹空状态和源码守卫，不改接入来源、检索策略、运行时事件或记忆加载边界；Further localized mixed Chinese empty-state copy in the meeting v2 ingestion and retrieval card; this batch changes the connector ingestion / retrieval empty state plus source guards only and leaves ingestion sources, retrieval policy, runtime events, and memory-loading boundaries unchanged |
...
| 2026-06-04 | 继续收口底层 UI select / switch primitive 的双语 aria fallback；本批只改缺省无障碍标签文案，不改视觉样式、Radix 行为、调用方传入 label 或表单状态；Further localized bilingual aria fallbacks in the low-level UI select / switch primitives; this batch changes default accessibility-label copy only and leaves visual styling, Radix behavior, caller-provided labels, and form state unchanged |
...
| 2026-06-03 | 继续收口 recommendation feedback、UI primitive aria fallback、guidance preferences、home surface arrival banner 与 billing program catalog 文案中的中文混写；本批只改中文可见文案，不改推荐反馈动作、首页路由或分成规则结构；Further localized Chinese visible mixed-language copy in recommendation feedback, UI primitive aria fallback, guidance preferences, home surface arrival banner, and billing program catalog surfaces; this batch changes visible copy only and leaves recommendation feedback actions, home routing, and revenue-rule structures unchanged |

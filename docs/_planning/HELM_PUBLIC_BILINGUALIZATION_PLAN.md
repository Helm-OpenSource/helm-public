...
3. 抽查 264 个 `.tsx` 文件后，少量硬编码英文 UI 标签已修正；workspace runtime
   ...
   复核卡片 / 步骤列表、recommendation feedback、UI primitive aria fallback、home surface arrival banner
...
3. After sampling 264 `.tsx` files, a small set of hard-coded English UI labels
   ...
   recommendation feedback, UI primitive aria fallback, home surface arrival banner, and billing program
...
| 2026-06-03 | 继续收口 recommendation feedback、UI primitive aria fallback、home surface arrival banner 与 billing program catalog 文案中的中文混写；本批只改中文可见文案，不改推荐反馈动作、首页路由或分成规则结构；Further localized Chinese visible mixed-language copy in recommendation feedback, UI primitive aria fallback, home surface arrival banner, and billing program catalog surfaces; this batch changes visible copy only and leaves recommendation feedback actions, home routing, and revenue-rule structures unchanged |

...
| 主仓库 `DATABASE_URL` host | `${HELM_DB_HOST}` |
...
```
Error: Can't reach database server at `${HELM_DB_HOST}`
Please make sure your database server is running at `${HELM_DB_HOST}`.
```
...
| 阻断项 | 状态 |
|---|---|
| 无法建立到 `${HELM_DB_HOST}` 的 TCP 连接 | 已确认 |
...
1. **恢复 VPN / 网络 / DB allowlist**，使当前机器能够到达 `${HELM_DB_HOST}`

...
---
# Helm DingTalk OAuth Callback Runtime Foundation Plan V1
...
PR84 只做 DingTalk runtime OAuth callback foundation：

1. `start/callback` runtime
2. `code -> token -> user profile`
3. workspace-scoped identity binding
4. `providerType = DINGTALK_OAUTH` session truth
...
- DingTalk runtime OAuth callback route
- token exchange + user profile fetch
- current workspace user matching
- `providerType = DINGTALK_OAUTH` session write
...
1. DingTalk 已从 foundation-only truth 推到真实可运行的 callback runtime foundation
...
`start route -> callback route -> token -> user profile -> workspace binding -> createSession(providerType) -> connector metadata -> tenant-scoped audit -> settings/operator readout -> docs/guards/tests`
...
- PR84 baseline / report / README / docs / guards / tests
...
3. provider/source compatibility 如果不同步，新的 `DINGTALK_OAUTH` session 会自我污染 auth anomaly readout

## 9. 阶段计划

### Phase 0

- 复核 current-main 与 PR73 truth
- 状态：Completed

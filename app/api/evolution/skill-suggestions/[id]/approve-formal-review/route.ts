...
import { approveSkillFormalReview } from "@/lib/evolution/skill-suggestion.service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const suggestion = await approveSkillFormalReview({
    ...
    return errorResponse(
      isWorkspaceOwnershipError(error) ? error.message : serverErrorMessage(error, english ? "Failed to approve skill formal review" : "批准正式评审失败"),

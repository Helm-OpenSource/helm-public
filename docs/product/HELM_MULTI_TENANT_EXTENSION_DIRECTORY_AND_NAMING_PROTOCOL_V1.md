...
本公开协议约束 `helm-public` 内 extension / sample 的目录命名，确保公开仓库
可理解、可 fork，同时不暴露客户专属私有目录。

公开 extension 和 sample 必须使用 generic 或 synthetic 名称，例如：

```text
extensions/<sample-or-generic-pack>/
```

它们不得使用真实客户、tenant、deployment、私有 vendor、私有 domain 或生产环境
名称。客户 branding、客户 connector runtime configuration、客户环境默认值、客户
联系人、domain、host 或 deployment receipt 都属于私有 Overlay 仓库，不属于
`helm-public`。

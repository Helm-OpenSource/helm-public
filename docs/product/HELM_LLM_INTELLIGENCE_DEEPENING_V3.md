...
私有 model-shadow caller 必须显式使用
`observabilityPolicy: "metadata_only"`。该模式不把 caller `inputSummary` 或 provider
输出 / 错误详情交给 call-log，且返回结果中的 `rawOutput` 固定为 `null`；解析后的 strict
结构化输出仍返回给调用方。默认 `standard` 行为保持兼容，`metadata_only` 本身不授予
egress、provider、action、writeback、send、approval 或 memory promotion 权限。
...
| 日期 | 变化 |
|---|---|
| 2026-07-13 | 为私有 model-shadow 增加 additive `metadata_only` observability policy；不改变默认 provider 行为或任何动作权限。 |

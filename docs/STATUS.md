...
| 公开双语化与交付工程师友好状态 / Public bilingualization and delivery-engineer-friendly posture | 已成形为分阶段计划；P0 intake 与文档入口已落地，P1 product / operations / roadmap / launch / trial / legal docs 已推进，P2 sample / templates / receipts 正在推进；源码 / UI 已按 `zh-CN` 默认与 `en-US` 可切换机制审计，直接 JSX 英文标签扫描已降为 0，会议人工执行与正式写回运行时面板的目标文件 `english` 条件中文分支扫描为 0；仍不把内部 enum / trace key 当成需翻译文本，且全工程源码 / UI 仍有剩余候选项；全仓双语完成尚未声明；Formed as a staged plan; P0 intake and docs-entry surfaces are landed, P1 product / operations / roadmap / launch / trial / legal docs have advanced, P2 sample / templates / receipts are underway; source / UI has been audited against the `zh-CN` default and `en-US` switchable mechanism, the direct JSX English-label scan is now 0, and the target-file `english` conditional scan for meeting human-execution and official-write runtime panels is now 0; internal enum / trace keys are not treated as translatable text, and full-project source / UI candidates still remain; full-repo bilingual completion is not claimed | [Public bilingualization plan](./_planning/HELM_PUBLIC_BILINGUALIZATION_PLAN.md), `.github/ISSUE_TEMPLATE/`, `.github/pull_request_template.md`, `docs/product/`, `docs/operations/`, `docs/roadmap/`, `docs/launch/`, `docs/pilot/`, `docs/legal/`, `extensions/case-management-sample/`, `lib/i18n/config.ts`, `lib/i18n/messages.ts`, `features/meetings/meeting-v2-human-action-execution-card.tsx`, `features/meetings/meeting-v2-official-write-card.tsx` |
